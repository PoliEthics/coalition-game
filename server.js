    ...
    socket.on('tallyVotes', () => {
        ...
        game.factions.forEach(f => {
            if (f.vote === true && passed) {
                f.tokens.capital += 1;
                f.voterApproval = Math.min(100, (f.voterApproval || 50) + 10);
        ...
        if (game.currentProposal && game.currentProposal.proposer) {
            ...
            if (proposer) {
                if (passed) {
                    ...
                    proposer.voterApproval = Math.min(100, (proposer.voterApproval || 50) + 10);
                    console.log(`  ✅ Proposer ${proposer.name}: +2 tokens, +10% approval`);
                } else {
                    proposer.voterApproval = Math.max(0, (proposer.voterApproval || 50) - 10);
                    console.log(`  ❌ Proposer ${proposer.name}: -10% approval (proposal failed)`);
                }
            }
        }
        ...
        if (policyId && policyEffects) {
            ...
            game.factions.forEach(faction => {
                ...
                // Check if all main objectives complete for +20% bonus
                const mainObjectives = faction.objectives.filter(o => !o.text.includes('BONUS'));
                const completedCount = mainObjectives.filter(o => o.completed).length;
                const failedCount = mainObjectives.filter(o => o.type === 'maintain_metric' && o.failed).length;
                
                // Award bonus only if ALL main objectives are actually completed
                const allMainComplete = mainObjectives.every(o => {
                    if (o.type === 'maintain_metric') return !o.failed;
                    if (o.type === 'final_metric') return true; // Will check at end
                    return o.completed;
                });
                
                console.log(`   📊 ${faction.name}: ${completedCount}/${mainObjectives.length} completed, ${failedCount} failed`);
                
                if (allMainComplete && !faction.objectivesBonus && completedCount === mainObjectives.length - (mainObjectives.filter(o => o.type === 'final_metric').length)) {
                    faction.objectivesBonus = true;
                    faction.voterApproval = Math.min(100, (faction.voterApproval || 50) + 10);
                    console.log(`   🏆 ${faction.name} completed ALL MAIN OBJECTIVES! +10% approval (now ${faction.voterApproval}%)`);
                }
            });
        }
